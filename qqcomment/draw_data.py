...
class draw_data():
    ...
    def draw_three_day(self, temp_k):
        from matplotlib import pyplot as plt
        import numpy as np
        alphy = 3 * 24 * (60 / temp_k)
        begin_time = u"2015-09-25"
        last_time = u"2015-09-27"
        #plt.title("2015-09-25 to 2015-09-28")
        plt.title(begin_time + u" 到 " + last_time)
        plt.xlabel("per " + str(temp_k) + " mins")
        color_list = ['b-','g-','r-','c-','m-','y-','k-'] 
        titile_item = self.db.TitleItem.find(
            {"num": { "$gt": 3000 }, "title_time": { "$gt": begin_time, "$lt": last_time } }
        ).sort([("title_time", pymongo.ASCENDING)])
        color_l = 0
        title_i = 0
        for ti in titile_item:
            ...
            plt.plot(x_time.keys(), x_time.values(), color_list[color_l%7])
            plt.text(post_time+alphy, x_time[post_time+alphy-1], ti["title_content"])
            color_l += 1
        plt.show()
        
if __name__ == "__main__":
    mydraw = draw_data()
    mydraw.draw_three_day(10)         
